...
```{r}
  ...
  breed_rank_all %>%
    ...
    pivot_longer(c("2013 Rank" : "2020 Rank"), names_to = "year", values_to = "rank") %>% 
  ...
  ggplot(aes(y = fct_reorder(Breed_rect, med_rank, .desc = TRUE), x = year)) +
    ...
    geom_line(aes(y = Breed_rect))
```

This graph has the order messed up because of the NA values for the Miniature American Shepherd. I think another way to make this graph more useful would be to 

```{r}
  view(left_join(breed_traits, breed_rank_all, by = str_squish(Breed)))
```
